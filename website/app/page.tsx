...
import { ArrowRight, BadgeCheck, Calculator, CircleDollarSign, CloudCog, Fuel, Languages, LockKeyhole, PiggyBank, ReceiptText, ShieldCheck, Sparkles, UserRoundCheck, WalletCards, Zap } from 'lucide-react';
import appData from '@/content/app-data.json';

const features = [
  { icon: WalletCards, title: 'Money in one view', text: 'Track everyday expenses, monthly budgets and savings goals from a focused dashboard.' },
  ...
  { icon: PiggyBank, title: 'Savings and Udhaar', text: 'Set savings goals and keep informal lending and borrowing organised in simple ledgers.' },
  ...
  { icon: ShieldCheck, title: 'Privacy by design', text: 'Guest activity stays on your device, while supported signed-in data uses encrypted sync.' },
];

const tools = [[Zap, 'Electricity'], [Fuel, 'Fuel'], [Calculator, 'Salary tax'], [PiggyBank, 'Savings'], [CircleDollarSign, 'Zakat']] as const;
...
export default function Home() {
  return <main>
    ...
    <section className="section security-section" id="security"><div className="shell security-card"><div className="security-mark"><LockKeyhole /></div><div><span className="kicker">Your information, handled carefully</span><h2>Private by default. Sync only with an account.</h2></div><div className="security-points"><p><UserRoundCheck /> Guest activity stays on the device and the session continues until you sign in or log out.</p><p><ShieldCheck /> Account data is protected in transit and encrypted for supported cloud sync.</p><p><LockKeyhole /> You can export an encrypted local backup and manage your account data from the app.</p></div><Link className="button light" href="/privacy">See exactly how data is handled <ArrowRight size={18} /></Link></div></section>
